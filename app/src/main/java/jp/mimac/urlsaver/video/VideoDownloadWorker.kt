...
class VideoDownloadWorker(
    ...
    private fun hasEnoughStorage(directory: File?, entryBytes: Long, incomingBytes: Long): Boolean {
        if (directory == null) return false
        if (entryBytes > MediaNetworkPolicy.MAX_ENTRY_MEDIA_BYTES - incomingBytes) return false
        return directory.usableSpace >= incomingBytes + MediaNetworkPolicy.MIN_FREE_BYTES
    }

    companion object {
        const val KEY_VIDEO_ASSET_ID = "videoAssetId"
        fun uniqueWorkName(assetId: Long): String = "video-download:$assetId"
        ...
        private fun hasAllowedUrlShape(url: String): Boolean {
            val uri = runCatching { URI(url) }.getOrNull() ?: return false
            val scheme = uri.scheme?.lowercase(Locale.US) ?: return false
            val host = uri.host?.lowercase(Locale.US) ?: return false
            return scheme == "https" && uri.userInfo == null && !MediaNetworkPolicy.isBlockedHostLiteral(host)
        }

        private fun normalizeDownloadUrl(url: String): String {
            return MediaNetworkPolicy.normalizeUrl(url)
        }
    }
}
