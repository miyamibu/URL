...
begin
    ...
    alter table public.promo_invite_code_events
        ...
        check (event in ('issued', 'email_sent', 'email_failed', 'revoked', 'redeemed', 'failed'));
exception
...
grant select, insert, update, delete on table public.promo_invite_codes to service_role;
grant select, insert, update, delete on table public.promo_invite_code_events to service_role;
...
revoke all on function private.hash_promo_invite_code(text) from public;
grant execute on function private.hash_promo_invite_code(text) to service_role;
