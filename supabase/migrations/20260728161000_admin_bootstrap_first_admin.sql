...
-- Bootstrap is intentionally a one-time, service-role-only operation. The
-- Web admin must not race two allowlisted users into multiple owner rows.
...
begin
    ...
    -- Keep the empty-table check and the first insert atomic when two
    -- allowlisted users sign in at the same time.
