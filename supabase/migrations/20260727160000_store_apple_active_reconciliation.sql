-- Apple App Store Server API active reconciliation.
--
-- 20260727130000_store_active_reconciliation.sql is already applied in some
-- environments. Keep that migration immutable and replace its functions here.
-- The Edge Function verifies Apple's response and sends only normalized fields,
-- the signed transaction SHA-256, and the observed timestamp to these RPCs.
...
begin
    ...
    if normalized_token_hash is not null then
        raise exception 'apple_purchase_token_not_allowed';
    end if;

    if normalized_transaction_id is not null
       and event_row.store_transaction_id is not null
       and event_row.store_transaction_id <> normalized_transaction_id then
    ...
    select verification.*
     ...
     where verification.store_platform = 'app_store'
       ...
       and (
           ...
           or verification.store_transaction_id = coalesce(normalized_transaction_id, event_row.store_transaction_id)
       )
     order by verification.verified_at desc nulls last, verification.updated_at desc
     limit 1
     for update;
    ...
    if verification_row.original_transaction_id is null
       or verification_row.original_transaction_id <> original_id then
        raise exception 'apple_original_transaction_binding_mismatch';
    end if;

    if event_row.user_id is not null and event_row.user_id <> verification_row.user_id then
        raise exception 'apple_user_binding_mismatch';
    end if;
...
create or replace function public.apply_store_subscription_reconciliation(
    ...
    p_signed_transaction_info_sha256 text
)
returns table (
    event_id uuid,
    result text,
...
declare
    ...
    normalized_signed_transaction_hash text := lower(nullif(trim(coalesce(p_signed_transaction_info_sha256, '')), ''));
    observed_at timestamptz := coalesce(p_observed_at, clock_timestamp());
...
begin
    ...
    if event_row.provider not in ('google_play', 'app_store') then
        raise exception 'store_provider_invalid';
    end if;
    if not event_row.signature_verified then
        raise exception 'provider_signature_unverified';
    ...
    if normalized_product_id is null or length(normalized_product_id) > 256 then
        raise exception 'store_product_binding_required';
    end if;
    if normalized_transaction_id is not null and length(normalized_transaction_id) > 256 then
        raise exception 'store_transaction_binding_invalid';
    end if;
    ...
    if observed_at is null then
        raise exception 'store_reconciliation_observed_at_required';
    end if;
    ...
    if event_row.provider = 'google_play' then
        ...
        if normalized_signed_transaction_hash is not null then
            raise exception 'google_signed_transaction_hash_not_allowed';
        end if;
        ...
        if normalized_transaction_id is null then
            raise exception 'apple_transaction_binding_required';
        end if;
        if normalized_signed_transaction_hash is null
           or normalized_signed_transaction_hash !~ '^[0-9a-f]{64}$' then
            raise exception 'apple_signed_transaction_hash_required';
        end if;
        ...
        select verification.*
         ...
         where verification.store_platform = 'app_store'
           and verification.status = 'verified'
           and verification.original_transaction_id = original_id
         order by verification.verified_at desc nulls last, verification.updated_at desc
         limit 1
         for update;

        if verification_row.id is null then
            raise exception 'apple_purchase_binding_not_found';
        end if;
        if verification_row.original_transaction_id is null
           or verification_row.original_transaction_id <> original_id then
            raise exception 'apple_original_transaction_binding_mismatch';
    ...
    if event_row.user_id is not null and event_row.user_id <> verification_row.user_id then
        raise exception 'user_binding_mismatch';
    end if;
    ...
    reconciliation_id := md5(concat_ws(
        ...
        event_row.provider,
        event_row.id::text,
        coalesce(normalized_token_hash, 'apple:' || event_row.subscription_key),
        ...
        observed_at::text,
        coalesce(normalized_signed_transaction_hash, '')
    ));
    ...
    select *
      ...
      from public.apply_store_subscription_notification(
          ...
          jsonb_build_object(
              'reconciliation', true,
              'source', case
                  when event_row.provider = 'app_store' then 'app_store_server_api'
                  else 'google_play_subscriptionsv2'
              end,
              'signed_transaction_info_sha256',
                  case when event_row.provider = 'app_store'
                       then normalized_signed_transaction_hash
                       else null
                  end
          )
      );
...
-- Keep the already-published seven-argument RPC callable by older internal
-- callers, but route it through the provider-aware implementation above.
create or replace function public.apply_store_subscription_reconciliation(
    p_event_row_id uuid,
    p_purchase_token_hash text,
    p_store_transaction_id text,
