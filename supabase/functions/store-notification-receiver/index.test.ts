  ...
  return {
    ...
    fetch: async (input: RequestInfo | URL): Promise<Response> => {
      ...
      if (url === "https://oauth2.googleapis.com/token") {
        return new Response(
          JSON.stringify({ access_token: "fixture-access-token" }),
          {
            status: 200,
...
Deno.test("Apple refund and billing mappings do not invent a regrant", () => {
  assertEquals(
    mapAppleNotification("REFUND", null, {}, {}).action,
    "revoke",
    "Apple refund action",
  );
  assertEquals(
    mapAppleNotification("REFUND", null, {}, {}).state,
    "refunded",
    "Apple refund state",
  );
  assertEquals(
    mapAppleNotification("DID_FAIL_TO_RENEW", "GRACE_PERIOD", {}, {}).action,
    "preserve",
    "Apple grace action",
  );
  assertEquals(
    mapAppleNotification("DID_FAIL_TO_RENEW", "GRACE_PERIOD", {}, {}).state,
    "grace_period",
    "Apple grace state",
  );
  assertEquals(
    mapAppleNotification("UNKNOWN_PROVIDER_EVENT", null, {}, {}).action,
    "noop",
    "unknown action",
  );
});
...
Deno.test("Google RTDN validates OIDC claims, package, product, and token binding", async () => {
  ...
  assertEquals(
    command.storeTransactionId,
    "GPA.0000-0000-0000-00000",
    "authoritative order ID",
  );
  assertEquals(
    command.purchaseTokenHash,
    await sha256Hex("fixture-purchase-token"),
    "Google token hash",
  );
  assert(
    !JSON.stringify(command).includes("fixture-purchase-token"),
    "raw purchase token must not enter command",
  );
});

Deno.test("Google paused and on-hold authoritative states revoke the grant", async () => {
  for (
    const [providerState, expectedState, expiryTime] of [
    ...
    assertEquals(
      command.action,
      "revoke",
      `${providerState} revokes the grant`,
    );
    assertEquals(
      command.subscriptionState,
      expectedState,
      `${providerState} state`,
    );
  }
...
Deno.test("Google canceled state preserves only until authoritative expiry", async () => {
  ...
  assertEquals(
    command.action,
    "revoke",
    "expired canceled state revokes the grant",
  );
  assertEquals(
    command.subscriptionState,
    "canceled",
    "canceled state is retained",
  );
});
...
Deno.test("Google RTDN revoke and full refund are the only notification-only grant removals", async () => {
  assertEquals(
    mapGoogleSubscriptionNotification(12).action,
    "revoke",
    "Google revoke action",
  );
  assertEquals(
    mapGoogleSubscriptionNotification(13).action,
    "revoke",
    "Google expiry action",
  );
  assertEquals(
    mapGoogleSubscriptionNotification(5).action,
    "revoke",
    "Google on-hold action",
  );
  assertEquals(
    mapGoogleSubscriptionNotification(5).state,
    "on_hold",
    "Google on-hold state",
  );
  assertEquals(
    mapGoogleSubscriptionNotification(6).state,
    "grace_period",
    "Google grace state",
  );
  assertEquals(
    mapGoogleSubscriptionNotification(10).action,
    "revoke",
    "Google paused action",
  );
  assertEquals(
    mapGoogleSubscriptionNotification(10).state,
    "paused",
    "Google paused state",
  );
