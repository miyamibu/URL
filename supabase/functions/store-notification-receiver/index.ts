...
type GooglePushTokenVerifier = (token: string, audience: string) => Promise<JWTPayload>;
...
export type GoogleJwtProvider = (credentials: GoogleServiceAccountCredentials) => Promise<string>;
...
export async function handleRequest(request: Request): Promise<Response> {
  ...
  try {
    ...
    const provider = stringValue(bodyRecord.provider) || inferProvider(bodyRecord);
    const command = provider === "app_store"
      ? await decodeAppleNotification(bodyRecord)
      : provider === "google_play"
      ? await decodeGoogleNotification(bodyRecord, request.headers.get("authorization") ?? "")
      : (() => {
        throw new Error("provider_required");
      })();

    if (!command || command.eventType === "test") {
      return jsonResponse({ status: "ignored", reason: "provider_test_notification" }, 202);
    }

    const supabaseUrl = requiredEnv("SUPABASE_URL");
    const serviceRoleKey = requiredEnv("SUPABASE_SERVICE_ROLE_KEY");
    const result = await applyStoreNotification(supabaseUrl, serviceRoleKey, command);
    return jsonResponse({
    ...
    const status = errorCode.includes("signature") || errorCode.includes("oidc") || errorCode.includes("authorization")
      ? 401
...
export async function decodeAppleNotification(
  ...
  if (bundleId !== resolvedOptions.bundleId || bundleId !== CANONICAL_APP_STORE_BUNDLE_ID) {
  ...
  if (signedTransactionInfo) {
    transaction = await resolvedVerifier.verifyAndDecodeTransaction(signedTransactionInfo);
  }
  let renewalInfo: unknown = null;
  const signedRenewalInfo = stringValue(data.signedRenewalInfo);
  if (signedRenewalInfo) {
    renewalInfo = await resolvedVerifier.verifyAndDecodeRenewalInfo(signedRenewalInfo);
  }
...
export function normalizeAppleDecodedNotification(
  ...
  if (!notificationId || !notificationType) throw new Error("apple_notification_identity_required");
  if (notificationType === "TEST") return null;
  if (!Object.keys(transaction).length) throw new Error("apple_transaction_binding_required");
  ...
  const appAccountToken = stringValue(transaction.appAccountToken).toLowerCase();
  if (!transactionId || !originalTransactionId || !productId) {
    throw new Error("apple_transaction_binding_required");
  }
  if (!validUuid(appAccountToken)) throw new Error("apple_user_binding_required");

  const occurredAt =
    isoFromMillis(notification.signedDate) ??
  ...
  const mapping = mapAppleNotification(notificationType, subtype, transaction, renewal);
  if (mapping.action === "activate" && !expiresAt) {
    throw new Error("apple_authoritative_expiry_required");
  }

  const revokeExpiry =
    isoFromMillis(transaction.revocationDate) ??
    expiresAt ??
    occurredAt;
  return {
    provider: "app_store",
...
export async function verifyAppleReconciliationTransaction(
  ...
  if (!signedTransactionInfo || signedTransactionInfo.length > MAX_NOTIFICATION_BODY_BYTES) {
    throw new Error("apple_transaction_binding_required");
  }
  const resolvedOptions = options ?? appleOptionsFromEnv();
  const resolvedVerifier = verifier ?? await appleVerifier();
  ...
  try {
    decoded = recordValue(await resolvedVerifier.verifyAndDecodeTransaction(signedTransactionInfo));
  } catch {
  ...
  if (expected.originalTransactionId && originalTransactionId !== expected.originalTransactionId) {
...
export async function decodeGoogleNotification(
  body: Record<string, unknown>,
  authorizationHeader: string,
  options?: Omit<GoogleEnvelopeOptions, "authorizationHeader">,
): Promise<StoreNotificationCommand> {
  const packageName = options?.packageName ?? requiredEnv(GOOGLE_PLAY_ENV_NAMES.packageName);
  validateGooglePackageName(packageName);
  const resolvedOptions: GoogleEnvelopeOptions = {
    authorizationHeader,
    packageName,
    audience: options?.audience ?? requiredEnv(GOOGLE_PLAY_ENV_NAMES.pushAudience),
...
export async function normalizeGooglePubSubEnvelope(
  ...
  validateGooglePushClaims(claims, options.audience, options.expectedServiceAccountEmail);
  ...
  if (!notificationId || !encodedData) throw new Error("google_notification_identity_required");

  let notification: Record<string, unknown>;
  try {
    notification = recordValue(JSON.parse(new TextDecoder().decode(decodeBase64(encodedData))));
  } catch {
  ...
  const occurredAt = isoFromMillis(eventTimeMillis) ?? isoFromDateString(message.publishTime);
  ...
  const pendingRefund = recordValue(notification.pendingRefundReviewNotification);
  ...
  if (Object.keys(subscription).length > 0) {
    ...
    return {
      provider: "google_play",
      notificationId,
      eventId: `google:${purchaseTokenHash}:${notificationType}:${String(eventTimeMillis ?? occurredAt)}`,
      ...
      detail: safeDetail({
        ...
        eventTimeMillis: typeof eventTimeMillis === "number" || typeof eventTimeMillis === "string"
  ...
  const purchaseToken = stringValue(voided.purchaseToken ?? oneTime.purchaseToken ?? pendingRefund.purchaseToken);
  ...
  if (Object.keys(voided).length > 0) {
    ...
    return {
      provider: "google_play",
      notificationId,
      eventId: `google:${purchaseTokenHash}:voided:${String(eventTimeMillis ?? occurredAt)}`,
  ...
  return {
    provider: "google_play",
    notificationId,
    eventId: `google:${purchaseTokenHash}:pending-refund:${String(eventTimeMillis ?? occurredAt)}`,
    eventType: Object.keys(oneTime).length > 0 ? "one_time_product" : "pending_refund_review",
    subscriptionKey: `google_play:${purchaseTokenHash}`,
    purchaseTokenHash,
    storeTransactionId: null,
...
export async function lookupGooglePlaySubscription(
  ...
  const serviceAccount = options.serviceAccount ?? googlePlayServiceAccountFromEnv();
  ...
  const accessToken = await requestGooglePlayAccessToken(requestFetch, assertion);
  const endpoint = `${GOOGLE_PLAY_DEVELOPER_API_BASE_URL}/applications/${encodeURIComponent(packageName)}` +
...
export function normalizeGooglePlaySubscription(
  ...
  if (stringValue(resource.kind) !== "androidpublisher#subscriptionPurchaseV2") {
  ...
  if (!Number.isNaN(expiryMillis)) {
    ...
    const stateRequiresFutureExpiry = providerState === "SUBSCRIPTION_STATE_ACTIVE" ||
      providerState === "SUBSCRIPTION_STATE_IN_GRACE_PERIOD";
    const stateRequiresPastExpiry = providerState === "SUBSCRIPTION_STATE_EXPIRED";
...
function mapGooglePlaySubscriptionState(
  providerState: string,
): { action: StoreNotificationCommand["action"]; state: StoreSubscriptionState } | null {
...
): { action: StoreNotificationCommand["action"]; state: StoreSubscriptionState } {
...
async function appleVerifier(): Promise<AppleVerifierLike> {
...
async function createAppleVerifier(): Promise<AppleVerifierLike> {
  ...
  const environment = options.environment === "sandbox" ? Environment.SANDBOX : Environment.PRODUCTION;
...
async function defaultVerifyGooglePushToken(token: string, audience: string): Promise<JWTPayload> {
  if (!googleJwks) {
    googleJwks = createRemoteJWKSet(new URL(
      Deno.env.get(GOOGLE_PLAY_ENV_NAMES.oidcJwksUrl)?.trim() || GOOGLE_PUBSUB_JWKS_URL,
    ));
  }
...
function validateGooglePushClaims(
  ...
  if (!GOOGLE_ISSUERS.has(issuer)) throw new Error("google_oidc_issuer_invalid");
  const audiences = Array.isArray(claims.aud) ? claims.aud : [claims.aud];
  if (!audiences.includes(audience)) throw new Error("google_oidc_audience_invalid");
  if (stringValue(claims.email) !== expectedServiceAccountEmail) {
    throw new Error("google_oidc_service_account_email_invalid");
  }
  if (claims.email_verified !== true) throw new Error("google_oidc_email_unverified");
...
function bearerToken(header: string): string {
  if (!header.toLowerCase().startsWith("bearer ")) throw new Error("google_authorization_required");
...
function publicError(error: unknown): string {
  const code = error instanceof Error ? error.message : "store_notification_failed";
