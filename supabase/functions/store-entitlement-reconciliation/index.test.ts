...
// Async fakes intentionally implement the same Promise contracts as production.
import {
...
Deno.test("reconciliation event ids are UUID-only, deduplicated, and bounded", () => {
  const values = normalizeEventIds([
    EVENT_A,
    EVENT_A.toUpperCase(),
    EVENT_B,
    "not-an-id",
  ...
  await withEnv({ STORE_RECONCILIATION_SECRET: "s".repeat(32) }, async () => {
    ...
    try {
      const response = await handleRequest(
        new Request("https://reconcile.test", {
          method: "POST",
    ...
    try {
      ...
      assert(
        calls[0].url.endsWith(
          "/rest/v1/rpc/reconcile_store_subscription_notification",
        ),
        "dedicated RPC",
      );
      assertEquals(
        JSON.parse(calls[0].body).p_event_row_id,
        EVENT_A,
        "event id only",
      );
...
Deno.test("Google active reconciliation hashes and binds the temporary token before lookup", async () => {
  ...
  const calls: string[] = [];
  await withEnv(
    { GOOGLE_PLAY_PACKAGE_NAME: "jp.miyamibu.urlalbum" },
    async () => {
      const result = await reconcileCurrentStoreState(
        ...
        { eventId: EVENT_A, purchaseToken: token },
        {
          prepareTarget: async (_url, _key, eventId, binding) => {
            ...
            assert(
              binding !== undefined,
              "temporary token binding is passed to DB target lookup",
            );
            assertEquals(
              binding.purchaseTokenHash,
              tokenHash,
              "stored token hash binding",
            );
            return {
              event_id: EVENT_A,
          ...
          lookupGoogle: async (packageName, productId, purchaseToken) => {
            ...
            assertEquals(
              purchaseToken,
              token,
              "raw token is used only for provider lookup",
            );
            return {
          ...
          applySnapshot: async (_url, _key, snapshot) => {
            assertEquals(
              snapshot.purchaseTokenHash,
              tokenHash,
              "only token hash reaches DB apply",
            );
            assert(
              !JSON.stringify(snapshot).includes(token),
              "raw token must not reach DB apply",
            );
            return {
              event_id: EVENT_A,
              result: "applied",
      ...
      assertEquals(
        result.mode,
        "google_current_state",
        "Google reconciliation mode",
      );
      assertEquals(
        result.verification,
        "verified",
        "Google reconciliation verification",
      );
  ...
  await withEnv(
    ...
    async () => {
      try {
        ...
        assertEquals(
          error.message,
          "google_purchase_token_binding_mismatch",
...
Deno.test("Apple active reconciliation verifies current state and applies the normalized snapshot", async () => {
    ...
    },
    {
      ...
      verifyAppleBinding: async (signedTransactionInfo, expected) => {
        assertEquals(
          ...
          "fixture-signed-transaction-info",
          "temporary JWS binding",
        );
        assertEquals(
          expected.originalTransactionId,
          "apple-original-002",
          "Apple original transaction binding",
        );
        return {
          transactionId: "apple-transaction-002",
      ...
      applySnapshot: async (_url, _key, snapshot) => {
        ...
        assertEquals(
          snapshot.purchaseTokenHash,
          null,
          "Apple reconciliation has no purchase token",
        );
        assertEquals(
          snapshot.storeTransactionId,
          "apple-transaction-003",
          "latest Apple transaction",
        );
        assertEquals(
          snapshot.signedTransactionInfoHash,
          "a".repeat(64),
          "JWS hash only reaches DB",
        );
        return {
          event_id: EVENT_B,
          result: "applied",
  ...
  assertEquals(
    result.verification,
    "verified",
    "Apple current state verification",
  );
  ...
  assertEquals(
    resolvedLookupTarget.originalTransactionId,
    "apple-original-002",
    "Apple target original transaction",
  );
  assertEquals(
    resolvedLookupTarget.productId,
    "urlsaver.standard.monthly",
    "Apple target product",
  );
  assertEquals(
    resolvedLookupTarget.userId,
...
Deno.test("Apple Server API lookup verifies response identity and selects the newest bound transaction", async () => {
  ...
  await withEnv({
    ...
    const snapshot = await lookupAppleSubscription(
      "apple-transaction-003",
      target,
      {
        nowMillis,
        ...
        verifyTransaction: async (signedTransactionInfo, expected) => {
          assertEquals(
            expected.originalTransactionId,
            null,
            "API verifies JWS before binding filter",
          );
    ...
    assertEquals(
      ...
      "apple-transaction-003",
      "newest transaction selected",
    );
    assertEquals(snapshot.status, 1, "Apple status selected");
    assertEquals(
      snapshot.action,
      "activate",
      "active Apple status maps to activate",
    );
    assertEquals(
      snapshot.subscriptionState,
      "active",
      "active Apple status maps to active",
    );
    assert(
      snapshot.signedTransactionInfoHash.length === 64,
      "only JWS hash is retained",
    );
  });
});
...
Deno.test("Apple subscription status mapping revokes expired retry and preserves only valid grace time", () => {
  const nowMillis = Date.parse("2026-07-27T00:00:00.000Z");
  assertEquals(
    mapAppleSubscriptionStatus(3, "2026-07-28T00:00:00.000Z", null, nowMillis)
      .action,
    "preserve",
    "billing retry remains active until its authoritative expiry",
  );
  assertEquals(
    mapAppleSubscriptionStatus(4, "2026-07-26T00:00:00.000Z", null, nowMillis)
      .action,
    "revoke",
...
Deno.test("reconciliation bindings reject mixed provider payloads and raw token is not normalized into output", () => {
  ...
  try {
    ...
    rejected = error instanceof Error &&
      error.message === "reconciliation_binding_provider_ambiguous";
  }
