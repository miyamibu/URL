...
import {
  ...
  type StoreReconciliationTarget,
...
const APPLE_SERVER_API_SANDBOX_BASE_URL = "https://api.storekit-sandbox.apple.com";
...
export type AppleServerApiLookupOptions = {
  fetch?: typeof fetch;
  jwtProvider?: (config: AppleServerApiJwtConfig, nowSeconds: number) => Promise<string>;
...
export type ActiveReconciliationResult = Omit<StoreNotificationResult, "result"> & {
...
export async function handleRequest(request: Request): Promise<Response> {
  ...
  if (!eventIds.length) return jsonResponse({ error: "event_ids_required" }, 400);
  ...
  try {
    ...
    for (const eventId of eventIds) {
      ...
      results.push(
        binding
          ? await reconcileCurrentStoreState(supabaseUrl, serviceRoleKey, binding)
          : await reconcileStoreNotification(supabaseUrl, serviceRoleKey, eventId),
      );
    }
...
export function normalizeBindings(
  ...
  for (const item of value) {
    ...
    if (!eventId || !allowed.has(eventId)) throw new Error("reconciliation_binding_event_invalid");
    ...
    if (signedTransactionInfo) binding.signedTransactionInfo = signedTransactionInfo;
...
export async function reconcileCurrentStoreState(
  supabaseUrl: string,
  ...
  if (binding.purchaseToken) {
    return reconcileGoogleCurrentState(supabaseUrl, serviceRoleKey, binding, dependencies);
  }
  return reconcileAppleCurrentState(supabaseUrl, serviceRoleKey, binding, dependencies);
}
...
async function reconcileGoogleCurrentState(
  ...
  const prepareTarget = dependencies.prepareTarget ?? prepareStoreSubscriptionReconciliation;
  const target = await prepareTarget(supabaseUrl, serviceRoleKey, binding.eventId, {
    purchaseTokenHash,
  });
  if (target.provider !== "google_play") throw new Error("google_reconciliation_provider_mismatch");
  if (target.binding_status !== "verified" || target.purchase_token_hash !== purchaseTokenHash) {
  ...
  const lookup = dependencies.lookupGoogle ?? (
    (name: string, product: string, token: string) => lookupGooglePlaySubscription(name, product, token)
  );
  const snapshot = await lookup(packageName, productId, purchaseToken);
  if (snapshot.productId !== productId) throw new Error("google_product_binding_mismatch");

  const applySnapshot = dependencies.applySnapshot ?? applyStoreSubscriptionReconciliation;
...
async function reconcileAppleCurrentState(
  ...
  if (target.provider !== "app_store") throw new Error("apple_reconciliation_provider_mismatch");
  if (target.binding_status !== "verified") {
    throw new Error(target.failure_reason ?? "apple_reconciliation_binding_not_verified");
  }
...
export async function lookupAppleSubscription(
  ...
  if (!anyTransactionId || !target.originalTransactionId || !target.productId || !target.userId) {
    throw new Error("apple_subscription_binding_required");
  }

  const config = requiredAppleServerApiConfig();
  const requestFetch = options.fetch ?? globalThis.fetch;
  const nowMillis = Number.isFinite(options.nowMillis) ? options.nowMillis as number : Date.now();
  ...
  const endpoint = `${appleServerApiBaseUrl(config.environment)}/inApps/v1/subscriptions/${encodeURIComponent(anyTransactionId)}`;
  ...
  for (const groupValue of groups) {
    ...
    for (const transactionValue of transactions) {
      ...
      const signedTransactionInfo = stringValue(transactionRecord.signedTransactionInfo);
      ...
      if (
        ...
        transaction.appAccountToken.toLowerCase() !== target.userId.toLowerCase()
      ) continue;
      candidates.push({
        status,
        transaction,
  ...
  if (!candidates.length) throw new Error("apple_subscription_binding_not_found");
  ...
  candidates.sort((left, right) => {
    ...
    return right.transaction.transactionId.localeCompare(left.transaction.transactionId);
  });
  ...
  const observedAt = selected.transaction.signedAt ?? selected.transaction.purchasedAt ??
...
export function mapAppleSubscriptionStatus(
  ...
  if (expiresAt !== null && expiryMillis === null) throw new Error("apple_authoritative_expiry_invalid");
  if (revokedAt !== null && revokedMillis === null) throw new Error("apple_revocation_date_invalid");
  ...
  switch (status) {
    ...
    case 3:
      return expiryMillis !== null && expiryMillis > nowMillis
        ? { action: "preserve", state: "billing_retry", eventExpiresAt: expiresAt }
        : { action: "revoke", state: "billing_retry", eventExpiresAt: expiresAt };
    case 4:
      return expiryMillis !== null && expiryMillis > nowMillis
        ? { action: "preserve", state: "grace_period", eventExpiresAt: expiresAt }
        : { action: "revoke", state: "grace_period", eventExpiresAt: expiresAt };
    case 5:
...
export async function createAppleServerApiJwt(
  ...
  if (!config.issuerId || !config.keyId || !config.privateKey || !config.bundleId) {
...
function validateAppleServerApiIdentity(
  ...
  config: { bundleId: string; environment: "sandbox" | "production"; appAppleId: number },
): void {
...
function positiveIntegerValue(value: unknown): number | null {
  const numberValue = typeof value === "number" ? value : typeof value === "string" && value.trim()
    ? Number(value)
    : Number.NaN;
...
function appleTransactionSortTime(transaction: AppleReconciliationTransaction): number {
  const values = [transaction.signedAt, transaction.purchasedAt, transaction.expiresAt];
...
function isAppleBindingMismatch(error: unknown): boolean {
  ...
  return /apple_(?:bundle_binding_mismatch|environment_mismatch|app_identity_mismatch|transaction_binding_mismatch|product_binding_mismatch|user_binding_mismatch|user_binding_required|transaction_binding_required)$/.test(message);
}
