...
export async function applyStoreNotification(
  ...
  const response = await fetch(
    `${
      supabaseUrl.replace(/\/+$/, "")
    }/rest/v1/rpc/apply_store_subscription_notification`,
    {
      method: "POST",
...
export async function prepareStoreSubscriptionReconciliation(
  ...
  const response = await fetch(
    `${
      supabaseUrl.replace(/\/+$/, "")
    }/rest/v1/rpc/prepare_store_subscription_reconciliation`,
    {
      method: "POST",
      headers: restHeaders(serviceRoleKey),
      body: JSON.stringify({
  ...
  if (!response.ok) {
    throw new Error(
      await readRpcErrorCode(response, "store_reconciliation_target_failed"),
    );
  }
  const body = await response.json().catch(() => null);
  const row = Array.isArray(body) ? body[0] : body;
  if (
    !row || typeof row.event_id !== "string" || typeof row.provider !== "string"
  ) {
  ...
  if (
    row.binding_status !== "verified" && row.binding_status !== "not_verified"
  ) {
  ...
  return {
    ...
    subscription_key: typeof row.subscription_key === "string"
      ? row.subscription_key
      : "",
    purchase_token_hash: typeof row.purchase_token_hash === "string"
      ? row.purchase_token_hash
      : null,
    store_product_id: typeof row.store_product_id === "string"
      ? row.store_product_id
      : null,
    store_transaction_id: typeof row.store_transaction_id === "string"
    ...
    failure_reason: typeof row.failure_reason === "string"
      ? row.failure_reason
      : null,
  };
}
...
export async function applyStoreSubscriptionReconciliation(
  ...
  const response = await fetch(
    `${
      supabaseUrl.replace(/\/+$/, "")
    }/rest/v1/rpc/apply_store_subscription_reconciliation`,
    {
      method: "POST",
      headers: restHeaders(serviceRoleKey),
      body: JSON.stringify({
...
async function readRpcResult(
  ...
  const row = Array.isArray(body) ? body[0] : body;
  if (
    !row || typeof row.event_id !== "string" || typeof row.result !== "string"
  ) {
...
async function readRpcErrorCode(
  response: Response,
  fallback: string,
): Promise<string> {
  ...
  for (const candidate of candidates) {
    if (
      typeof candidate === "string" && /^[a-z0-9_:-]{1,96}$/.test(candidate)
    ) return candidate;
  }
  return fallback;
}
