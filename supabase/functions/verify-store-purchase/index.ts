...
export function normalizeRequest(
...
): NormalizedStorePurchaseRequest {
  ...
  if (
    !["google_play", "app_store"].includes(storePlatform) || !storeProductId ||
    !purchaseToken
  ) {
  ...
  return {
    storePlatform:
      storePlatform as NormalizedStorePurchaseRequest["storePlatform"],
    storeProductId,
    purchaseToken,
    storeTransactionId,
  };
...
export function expectedPlanForProduct(productId: string): ProductPlan | null {
  ...
  return {
    plan: match[1] as ProductPlan["plan"],
    billingPeriod: match[2] as ProductPlan["billingPeriod"],
  };
}
...
async function activePlanForProduct(
  ...
  const response = await fetch(
    `${supabaseUrl}/rest/v1/subscription_products?select=plan,billing_period&store_platform=eq.${storePlatform}&store_product_id=eq.${
      encodeURIComponent(storeProductId)
    }&is_active=eq.true&limit=1`,
    { headers: restHeaders(serviceRoleKey) },
  );
  if (!response.ok) {
    throw new Error(`product_catalog_lookup_failed:${response.status}`);
  }
  ...
  if (
    !parsed || catalogPlan !== parsed.plan ||
    catalogPeriod !== parsed.billingPeriod
  ) return null;
...
async function verifyAppStorePurchase(
  ...
  if (
    request.storeTransactionId && transactionId !== request.storeTransactionId
  ) throw new Error("transaction_mismatch");
  if (productId !== `urlsaver.${expected.plan}.${expected.billingPeriod}`) {
    throw new Error("product_mismatch");
  }
  if (stringValue(payload.type) !== "Auto-Renewable Subscription") {
    throw new Error("app_store_product_type_mismatch");
  }
  ...
  if (
    !(await constantTimeStringEqual(
      stringValue(payload.appAccountToken).toLowerCase(),
      userId.toLowerCase(),
    ))
  ) {
...
function appleVerifier(): Promise<SignedDataVerifier> {
...
async function createAppleVerifier(): Promise<SignedDataVerifier> {
  ...
  const rootCertificates = await Promise.all(
    APPLE_ROOT_CERTIFICATES.map(async (certificate) => {
      ...
      if (actualHash !== certificate.sha256) {
...
async function verifyGooglePlayPurchase(
  ...
  const accountBinding = stringValue(
  ...
  const obfuscatedProfileId = stringValue(
  ...
  const matchingItem = lineItems.find((item: { productId?: unknown }) =>
    item?.productId === request.storeProductId
  );
  if (!matchingItem) throw new Error("product_mismatch");
  if (
    !["SUBSCRIPTION_STATE_ACTIVE", "SUBSCRIPTION_STATE_IN_GRACE_PERIOD"]
      .includes(String(body.subscriptionState))
  ) {
    throw new Error("subscription_not_active");
  }
  const expiresAt = stringValue(matchingItem.expiryTime);
  if (!expiresAt || !Number.isFinite(Date.parse(expiresAt))) {
    throw new Error("subscription_expiry_missing");
  }
  if (Date.parse(expiresAt) <= Date.now()) {
    throw new Error("subscription_expired");
  }
  const providerTransactionId =
    stringValue(matchingItem.latestSuccessfulOrderId) ||
    stringValue(body.latestOrderId);
  if (!providerTransactionId) throw new Error("transaction_missing");
  if (
    request.storeTransactionId &&
    request.storeTransactionId !== providerTransactionId
  ) {
    throw new Error("transaction_mismatch");
  }
  return {
...
async function recordVerification(
  ...
  if (existing?.id) {
    ...
    const updateResponse = await fetch(
      `${supabaseUrl}/rest/v1/store_purchase_verifications?id=eq.${
      ...
      }`,
      {
        ...
        body: JSON.stringify({
          ...
          store_transaction_id: verified?.storeTransactionId ||
            request.storeTransactionId || null,
    ...
    if (!updateResponse.ok) {
      throw new Error(`verification_record_failed:${updateResponse.status}`);
    }
    return existing.id;
  }

  const response = await fetch(
    `${supabaseUrl}/rest/v1/store_purchase_verifications`,
    {
      method: "POST",
  ...
  if (!response.ok) {
    throw new Error(`verification_record_failed:${await response.text()}`);
  }
  const rows = await response.json();
  const id = rows?.[0]?.id;
...
async function findExistingVerification(
  ...
  if (!byToken.ok) {
    throw new Error(`verification_lookup_failed:${byToken.status}`);
  }
  ...
  const byOriginal = await fetch(
    `${supabaseUrl}/rest/v1/store_purchase_verifications?select=id,user_id,status&store_platform=eq.${request.storePlatform}&original_transaction_id=eq.${
      encodeURIComponent(originalTransactionId)
    }&status=eq.verified&limit=1`,
    { headers: restHeaders(serviceRoleKey) },
  );
  if (!byOriginal.ok) {
    throw new Error(`verification_lookup_failed:${byOriginal.status}`);
  }
...
function publicFailureReason(message: string): string {
  if (message.startsWith("missing_env_")) {
    return "store_verification_not_configured";
  }
