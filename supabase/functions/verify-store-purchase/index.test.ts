...
Deno.test("purchase policy helpers reject unknown products and bind strings", async () => {
  assertEquals(
    expectedPlanForProduct("urlsaver.standard.monthly")?.plan,
    "standard",
    "standard product",
  );
  assertEquals(
    expectedPlanForProduct("urlsaver.pro.yearly")?.billingPeriod,
    "yearly",
    "pro product",
  );
  assertEquals(
    expectedPlanForProduct("urlsaver.free.monthly"),
    null,
    "unknown product",
  );
  assert(await constantTimeStringEqual("user-a", "user-a"), "equal binding");
  assert(
    !(await constantTimeStringEqual("user-a", "user-b")),
    "mismatched binding",
  );
...
Deno.test("malformed App Store JWS fails closed without granting", async () => {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (url.includes("/rest/v1/subscription_products?")) {
      return new Response(
        JSON.stringify([{ plan: "standard", billing_period: "monthly" }]),
        { status: 200 },
      );
    }
    ...
    if (
      url.endsWith("/rest/v1/store_purchase_verifications") &&
      init?.method === "POST"
    ) {
      return new Response(JSON.stringify([{ id: "verification-test-id" }]), {
        status: 201,
      });
    }
  ...
  try {
    const response = await handleRequest(
      new Request(
        "https://purchase-security-test.invalid/functions/v1/verify-store-purchase",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            storeProductId: "urlsaver.standard.monthly",
            purchaseToken:
              "eyJhbGciOiJFUzI1NiIsIng1YyI6WyJhdHRhY2tlciJdfQ.invalid.signature",
          }),
        },
      ),
    );
