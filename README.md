...
- shared tag sync を internal/debug ビルドで有効にする場合のみ、`local.properties` に以下を設定する
  ...
  - 問い合わせのアプリ内即送信を有効にする場合は、Supabase Edge Function `contact-support` をデプロイし、`contact.support.endpoint.url` または `URLSAVER_CONTACT_SUPPORT_ENDPOINT_URL` に公開Function URLを設定する
    - 例: `https://<project-ref>.supabase.co/functions/v1/contact-support`
    - `RESEND_API_KEY`、`CONTACT_SUPPORT_FROM`、`CONTACT_SUPPORT_TO`、`CONTACT_SUPPORT_HASH_SECRET`、`RESEND_WEBHOOK_SECRET` はSupabase secretsにだけ設定し、mobile appには入れない
    - ユーザー本人への控えメールは送らず、問い合わせ先メールへの `reply_to` としてユーザー入力メールを使う
  - iOS では `ruby ios/generate_xcodeproj.rb` 実行前に以下の環境変数を渡す
    ...
    - `URLSAVER_CONTACT_SUPPORT_ENDPOINT_URL=https://<project-ref>.supabase.co/functions/v1/contact-support`
...
- クリーンなテスト端末へ初回インストールする場合のみ:
  - `./scripts/install_on_device.sh --serial <adb-serial> --allow-fresh-install`
...
- `assembleDebug` は成功するのに実機へ入らない:
  ...
  - `INSTALL_FAILED_UPDATE_INCOMPATIBLE` は旧アプリIDや署名違いの可能性がある。通常端末ではアンインストールせず、対象が正準ID `jp.miyamibu.urlalbum` か、テスト端末でデータ削除が承認済みかを確認する
...
- [`docs/prompts/README.md`](docs/prompts/README.md): 現行の作業ルール・手順と旧プロンプトの参照方法
