...
const protectedPath = process.env.RINBAM_ADMIN_E2E_PROTECTED_PATH || "/";
...
test.describe("りんばむ admin release E2E contract", () => {
  ...
  test("server error and duplicate submit are surfaced without an external mutation", async ({ page }) => {
    ...
    await page.route(mutationRoute, async (route) => {
      ...
      await route.fulfill({
        status: requestCount === 1 ? errorStatus : 409,
        contentType: "application/json",
        body: JSON.stringify(requestCount === 1
          ? { ok: false, error: "fixture-error" }
          : {
              error: "duplicate_operation",
              operation: { operationId: payload.operationId, status: "started", code: null },
            }),
      });
    });
    ...
    expect(operationIds[0]).toMatch(/^[0-9a-f-]{36}$/i);
