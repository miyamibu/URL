...
check_contains "$privacy_html" "端末内消去が中断した場合" "privacy discloses interrupted on-device cleanup recovery"
check_contains "$privacy_html" "同じ要求が完了済みかどうかを安全に確認" "privacy discloses idempotent deletion status"
check_contains "$account_html" "端末内のデータと中断時の扱い" "account deletion explains on-device cleanup boundaries"
check_contains "$account_html" "同じ削除要求の状態を安全に確認できます" "account deletion explains idempotent status recovery"
