...
- App Store Connect Web authentication returned to the sign-in URL with `authResult=FAILED`. No credential, passkey, or two-factor action was performed by the coordinator.
...
- Exact version `1.0.19 (21)` was not operated through Appium/WebDriverAgent on the physical iPhone. Appium 3.5.0 with XCUITest driver 11.16.3 identified the USB device, WDA status was briefly reachable over the active CoreDevice tunnel, and a fresh WDA build, signature, install, and Runner launch succeeded. XCTest then reported `Timed out while enabling automation mode` and entered a macOS `Password:` prompt. No password was entered; coordinator-created WDA/Appium processes were stopped without uninstalling or resetting the app. Install/launch and archive/export/upload success are not physical-device UI proof.
