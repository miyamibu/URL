...
- エクスポート画面に `ChatGPTに聞く` を置き、自作タグ選択、対象URL/出力内容の確認、ChatGPT用ZIP生成、OS共有までを行えるようにする。
- ChatGPT用ZIPは、選択した自作タグのいずれかに該当する ACTIVE/local provenance のURLのうち、削除待ちでなく共有参照を持たないものだけを含める。
- ZIP生成前に既知パターンの伏せ字を適用した対象URL/出力内容、対象件数、除外件数/理由を表示し、0件の場合は生成しない。
- 既知のemail/phone/token-like/Supabase/JWT/local-pathパターンは検出して伏せ字にする。未知の秘密まで検出できるとは断言せず、ユーザーがpreviewを確認して未知の秘密が含まれていないことを明示確認してから生成/共有する。
