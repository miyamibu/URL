...
- ホーム右下の `タグ` / `アーカイブ` より上に独立した `AI` を置き、`ChatGPT` / `Gemini` / `Claude` / `DeepSeek` を選択できるようにする。選択後は自作タグ選択、対象確認、AI-safe ZIP生成からOS共有までを同じ契約で行い、エクスポート画面内のモード切替には戻さない。
...
- ChatGPT個人リンク同期カードは共有タグプロフィール画面に表示する。外部接続未設定時は「現在は利用できません」と表示し、操作不可とする。
- `公開ページも同期` 欄は出さない。
- 通常のAI向けZIP/JSON出力はエクスポート画面、AI provider手動共有はホームの独立した `AI` を正式な導線とする。
