...
- MCP/Store/Supabase live は未検証の外部ゲートであり、本書のローカル実装/検証とは分離する。
...
- 自作タグ名はダブルタップに依存せず、長押し・メニュー・アクセシビリティアクション等で変更できる。
...
- 同期中は小さく状態表示する。
- ChatGPT個人リンク同期カードは共有タグプロフィール画面に表示する。外部接続未設定時は「現在は利用できません」と表示し、操作不可とする。
...
- ホーム右下の `タグ` / `アーカイブ` より上に独立した `ChatGPT` を置き、自作タグ選択と1回の送信操作でChatGPT用ZIP生成からOS共有までを行えるようにする。エクスポート画面内のモード切替には戻さない。
- ChatGPT用ZIPは、選択した自作タグのいずれかに該当する ACTIVE/local provenance のURLのうち、削除待ちでなく共有参照を持たないものだけを含める。
- 専用ページは自作タグ選択、対象件数、除外件数、`ChatGPTに送る` を中心に簡潔にする。全URL一覧、長い説明、別のZIP作成ボタン、確認チェックは常設せず、対象0件の場合は生成しない。
- 既知のemail/phone/token-like/Supabase/JWT/local-pathパターンは検出して伏せ字にする。`ChatGPTに送る` の押下を表示対象件数に対する明示確認とし、生成直前にsnapshotを再検証して、選択や対象が変わった場合は共有しない。
- AndroidはChatGPTへの直接共有を試し、利用できなければ共有先選択を開く。iPhoneは共有シートを開く。
- Google Doc第13章の34項目は添付後のChatGPT側の活用例であって、りんばむ内の34機能ではない。ChatGPT用ZIPのREADMEには1〜34を個別列挙し、契約テストで全項目を照合する。りんばむは自作タグ選択、対象/出力内容確認、ZIP生成、OS共有だけを担当する。
- りんばむに質問入力欄を追加せず、質問文の自動入力/送信、OpenAI API/OAuth/login、MCP/provider接続も行わない。ChatGPTでの共有先選択、質問入力、送信はユーザーが行う。ChatGPT個人リンク同期/read-only MCPとは別機能として扱う。
- バックアップ/復元は別機能として扱う。
- 将来のバックアップ/復元追加余地は残す。
- ChatGPT個人リンク同期カードは共有タグプロフィール画面に表示する。外部接続未設定時は「現在は利用できません」と表示し、操作不可とする。
...
- ChatGPT個人リンク同期カードは共有タグプロフィール画面に表示する。外部接続未設定時は「現在は利用できません」と表示し、操作不可とする。
- `公開ページも同期` 欄は出さない。
- 通常のAI向けZIP/JSON出力はエクスポート画面、ChatGPT手動共有はホームの独立した `ChatGPT` を正式な導線とする。
- AI透明化entryは Android の `DEBUG && AI_TRANSPARENCY_ENABLED`、iOS の `DEBUG`/Info.plist flag に限定された Debug-only UI で、通常のRelease UIには出さない。
...
- 未完成/未運用の `公開ページも同期` などの連携欄を通常UIに出すこと。ChatGPT個人リンク同期カードは共有タグプロフィール画面に表示する現行仕様を除く。
