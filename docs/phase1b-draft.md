...
- `./gradlew connectedDebugAndroidTest` は手動検証として実行し、結果を記録する（CI hard gate には含めない）
...
- 一覧カードの表示文言は以下の方針で統一する（短文優先）。
  - `PENDING` -> `取得中`
  - `FAILED` -> `一時的に取得できません`（再取得導線あり）
  - `UNAVAILABLE` -> `自動取得できません` / `自動取得に制限あり`
  - `READY` -> 状態行なし
...
| `FAILED` | `一時的に取得できません` | `一時的に情報を取得できませんでした` | 再取得を促す |
| `UNAVAILABLE` | `自動取得できません`（制限時は `自動取得に制限あり`） | `このURLは自動取得できません`（制限時はサービス名付き） | 対象外/アクセス制限を短文補足 |
| `READY` | 表示なし | 表示なし | なし |
...
### 4.5 Main active 一覧のスワイプ操作（P0）
- Main の active 一覧カードは、タップ導線（詳細遷移）を維持したままスワイプ操作を提供する。
- ジェスチャー定義:
  - start-to-end（右）: `archive`
  - end-to-start（左）: `pending delete`（既存の DB-backed Undo フローを使用）
- 実装ガードレール:
  - Compose 側はジェスチャー検知と affordance 表示までに留める。
  - 実処理は既存 repository / ViewModel フロー（archive, markPendingDelete, Undo Snackbar, delete timer）を再利用する。
  - action が失敗した場合は既存 Snackbar event パターンで失敗通知し、silent failure を許容しない。
- 適用範囲:
  - Main の active 一覧のみ。
  - Archive 一覧の操作モデルは現行どおり（タップで詳細）。
...
- metadata 文言は内部状態語から分離した。
  - 一覧は `取得中` / `一時的に取得できません` / `自動取得できません` を使い分けた。
  - 詳細は retryable と unavailable を分離し、SNS 制限時はサービス名を含めた正直な説明にした。
...
- Detail では URL 技術情報ラベルを日本語化し、`originalUrl` / `normalizedUrl` / `displayUrl` の意味を明示した。
- メモ本文は `メモを編集` ボタン直下へ移動し、既存ダイアログ編集フローは維持した。
- Main / Archive / Detail の保存時刻表示は `createdAt` に統一した（並び順は維持）。
...
- 一覧は軽量改善に留めた。
  ...
  - 命名の誤解を避けるため、サービス絞り込みの状態名は `ListFilter*` へ統一した（挙動変更なし）。
  - 並び順大変更 / タグ / canonical 強化は未着手のまま維持。
- Main active 一覧へ実スワイプ操作を追加した。
  - 右スワイプ: archive
  - 左スワイプ: pending delete（DB-backed Undo）
  - action 実行失敗時は Snackbar で失敗を明示し、silent failure を許容しない。
  - 実処理は既存 repository / MainActivityViewModel の Undo・timer フローを再利用した。
...
- `./gradlew connectedDebugAndroidTest`: 失敗
  - blocker: `java.lang.NoSuchMethodException: android.hardware.input.InputManager.getInstance`（Pixel 9a - 16 / 12 件すべて同一）
- connected instrumentation は pass 扱いせず、手動検証未完了として記録

## 10. 実装時のガードレール（1b）
- 既存責務分離を維持する（Compose に業務ロジックを寄せない）。
- `MainActivityViewModel` の `consumeShareResult(intent, currentRoute)` の設計を維持する。
  - consume idempotence は extras ベース stable signature（event token 優先）で担保する。
- `DUPLICATE_ARCHIVED` 判定で `currentRoute == archive` の action 抑制を維持する。
- metadata の分類・再試行・Work key は維持しつつ、metadata 取得は service-aware に拡張する。
  - X: syndication -> oEmbed
  - YouTube: oEmbed + JSON-LD/description + card description 保存
