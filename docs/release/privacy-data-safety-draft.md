...
## Goal
現在の source baseline（Android `1.0.15 (versionCode=18)` / iOS `1.0.16 (build=18)`）から次回提出する privacy / Data safety 回答を、選択した release mode と照合できる状態にする。過去の提出証跡は履歴として残すが、現在の提出可否の証明には使わない。
...
- Account sign-in: enabled only when shared-tag cloud is enabled for the submitted binary.
 - Current repo status: `NO_GO_INTERNAL / NOT_VERIFIED`; manual ChatGPT handoff implementation tests and local disclosure review are local evidence only. Device E2E, signing, and external live/store verification remain separate manual gates.
