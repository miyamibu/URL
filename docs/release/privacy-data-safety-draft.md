...
| Account deletion | `docs/account-deletion.md` and `web/invite-link/account-deletion/index.html` exist for cloud release; `https://miyamibu.xyz/account-deletion/` returned HTTP 200 after Vercel deployment `dpl_GeSsSnoG2tUyNfkuAtgaHqtQrmBd` on 2026-07-26. | Public deletion route is reachable; enter the URL in store consoles and keep it aligned with account features. | DONE_PUBLIC_200 |
| Public privacy policy | `web/invite-link/privacy/index.html` source includes the manual ChatGPT handoff boundary; Vercel deployment `dpl_GeSsSnoG2tUyNfkuAtgaHqtQrmBd` was verified at `https://miyamibu.xyz/privacy/` on 2026-07-26. | Live privacy policy discloses paid plans, Google Play Billing, StoreKit, and the current handoff boundary without stale no-real-billing wording. | PUBLIC_VERIFIED / OWNER_STORE_REVIEW_REQUIRED |
