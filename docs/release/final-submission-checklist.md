...
- Current repo-local checks and public-Web contracts are tracked independently from release approval. Apple published version 1.0.18 (20) on 2026-08-25 with the submitted local-only copy and Japanese declaration. Current source 1.0.19 (21) adds the audited Share Extension fail-closed correction; its exact signed binary was archived, exported, verified, and uploaded successfully on 2026-08-25. Current status is `REPO_LOCAL_GO / APP_STORE_1_0_19_UPLOAD_GO / VERSION_CREATION_AND_SUBMISSION_AUTH_BLOCKED / DEVICE_UI_PARTIAL`; Apple authentication, version creation, build attachment, review, public propagation, sandbox purchase, and production integrations remain separate gates.
...
| iOS public version and archive | The official App Store URL showed public `1.0.18` on 2026-08-25, with copy explicitly identifying build 20 behavior. Current-source 1.0.19 (21) is separately Apple Distribution signed, exact-binary verified, and uploaded successfully; version creation/build attachment/review submission remain. | `PUBLIC_1_0_18_CONFIRMED / CURRENT_SOURCE_1_0_19_UPLOADED`. |
| Store declaration/listing | Google Play's public page directly shows the corrected collection/sharing/encryption/deletion summary. App Store 1.0.18 (20) is public with matching local-only description, What's New, and Japanese language; 1.0.19 metadata and review submission remain owner-authenticated Console actions. | `GOOGLE_DATA_SAFETY_PUBLIC / APP_STORE_1_0_18_PUBLIC / 1_0_19_SUBMISSION_AUTH_BLOCKED`. |
