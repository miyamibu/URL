...
- iOS artifact: `build/app-store/1.0.22-37-ci-compatible/りんばむ.ipa`; SHA-256 `dc3792cb09201e113d76a80dfc1e2668f43dd38b676452214df896ac86ea9266`. Export explicitly set `manageAppVersionAndBuildNumber=false`.
...
- Final release source: `941dbc36607c84d0bf84430859aac71bf489e62a`. The notification authorization bridge was adjusted for the older Swift/SDK in GitHub CI; the final iOS archive/export was rebuilt from that commit. The Android source is unchanged from `9ee610c2`.
- GitHub [Cross-platform CI](https://github.com/miyamibu/URL/actions/runs/34792611428) completed successfully for the final release source: Android, iOS app/tests/Share Extension, Python release scripts, all Supabase Deno tests, and local SQL/RLS/pgTAP. [Production feature contracts](https://github.com/miyamibu/URL/actions/runs/34792611441) also passed. The final iOS CI run executed 245 tests, 3 skipped, 0 failures.
- Apple: upload succeeded at 2026-09-14 09:30:58 JST; processing completed and build `37` (`d80bf281-6be2-4e70-9230-afe85ad1d93e`) was attached to `1.0.22`. At 09:40 JST one item was submitted. [Submission cf1a0433-f524-4380-8b74-314139d26774](https://appstoreconnect.apple.com/apps/6771251450/distribution/reviewsubmissions/details/cf1a0433-f524-4380-8b74-314139d26774) shows `審査待ち` / `WAITING_FOR_REVIEW` and `1.0.22 (37)`.
- Apple upload used the verified CI-compatible archive with the same export settings and fixed build number. The IPA hash above identifies the retained local export; Xcode did not retain the temporary IPA used by the upload operation. Archive/source identity, distribution verification, upload success, build processing, association, and the review receipt provide the recorded provenance.
- Google Play: the signed AAB was accepted in production release `19`. The support-device comparison showed no removed/new devices. One production change, `1.0.22 (37) リンク情報取得と共有の改善`, was sent at 2026-09-14 09:40 JST. [Submission 20](https://play.google.com/console/u/0/developers/6026364900028455951/app/4976058498782318560/publishing/submission-activity/20/details) shows `審査中` / `IN_REVIEW`. A transient `604EFA92` UI error appeared after the final click; a fresh page and submission history confirmed that the same request was received, so no second send or upload was made. Automated quick checks were still running at the initial receipt.
- Existing release choices were preserved: Apple automatic release and immediate all-user update; Google managed publishing off, 100% rollout to the existing country selection. Screenshots, preview video, privacy declarations, and account/security settings were preserved.
- Temporary test environment: the single owned simulator was removed after all local test processes ended; the protected Rinbam simulator remains and the XCTest clone set was empty. Original source, user data, signed artifacts and evidence remain preserved.
- Status: `STORE_SUBMISSIONS_RECEIVED / GOOGLE_IN_REVIEW / APPLE_WAITING_FOR_REVIEW`. Approval and public availability are external pending states. The final receipt-only commit does not change the submitted app source or release settings.
...
| iOS public version and archive | The official App Store URL showed public `1.0.18` on 2026-08-25, with copy explicitly identifying build 20 behavior. Current-source 1.0.19 (21) is Apple Distribution signed, exact-binary verified, uploaded, attached, and submitted under submission ID `4dded547-70a6-4da3-99c0-00e9156023b9`; status was `審査待ち` at 2026-08-26 10:20 JST. | `PUBLIC_1_0_18_CONFIRMED / CURRENT_SOURCE_1_0_19_REVIEW_PENDING`. |
| Store declaration/listing | Google Play's public page directly shows the corrected collection/sharing/encryption/deletion summary. App Store 1.0.18 (20) is public with matching local-only description, What's New, and Japanese language; 1.0.19 (21) has matching local-only metadata and review notes saved and submitted. | `GOOGLE_DATA_SAFETY_PUBLIC / APP_STORE_1_0_18_PUBLIC / 1_0_19_REVIEW_PENDING`. |
| iOS localization | Published version 1.0.18 (20) has Japanese packaging and the official page now reports `JA 日本語`. | `PUBLIC_LANGUAGE_CONFIRMED_FOR_1_0_18`. |
