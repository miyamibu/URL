...
| RC_INCLUDE_CANDIDATE | `app/src/main/java/jp/mimac/urlsaver/data/**`, including `LocalTagCollectionEntryRef.kt` and `app/schemas/jp.mimac.urlsaver.data.AppDatabase/13.json` | Tag behavior and legacy Collection schema compatibility affect saved URL data safety; Collection UI/write paths remain retired. | Treat as app-runtime RC candidate; stage only with migration/test review. |
