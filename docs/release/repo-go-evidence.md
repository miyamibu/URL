...
| Physical iPhone UI | PASS_WITH_PRIOR_APPIUM_EVIDENCE | Prior Appium/XCUITest evidence verified bundle `com.mibu.codebridge.ios` on physical UDID `00008101-00066D96340A001E`; `共有タグ`, `使い方`, `エクスポート`, `タグ`, and `アーカイブ` are present. Current-source build15 install/launch is separately verified, but a new post-parity full UI-operation session is pending RemoteXPC/WDA recovery. Evidence: `artifacts/ui-review/2026-07-15/iphone-home-appium-approved.png`; earlier scroll evidence remains under `artifacts/ui-review/2026-07-13/ios-appium/`. |
