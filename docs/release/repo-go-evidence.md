...
The historical evidence below is retained, but the current 2026-07-18 evidence proves the manual ChatGPT handoff through the normal ChatGPT composer on a canonical Android physical device, and through ZIP creation plus the iOS share sheet on a current-source physical iPhone. The current source baseline is Android `versionCode=18` / iOS `build=16`. Distribution signing, store upload/submission, OpenAI submission, production secret entry, and store-console review remain separate external/manual gates.
...
- Current-source physical iPhone proof covers Appium/WDA operation through ZIP creation and iOS share-sheet display. ChatGPT selection/composer proof is not available on that device because `com.openai.chatgpt` is not installed; historical build16 composer evidence is retained separately and is not promoted to current-source proof.
...
| iOS | CURRENT_SOURCE_SHARE_SHEET_PASS / CHATGPT_APP_NOT_INSTALLED / DISTRIBUTION_BLOCKED | Current source `com.mibu.codebridge.ios`, `1.0.15` build `16`, was operated on iPhone 12 UDID `00008101-00066D96340A001E` with Appium/WDA through tag selection, 3-item preview, confirmation, ZIP creation, and share-sheet display. `com.openai.chatgpt` is not installed on this iPhone, so ChatGPT selection/composer remains an external-device gate. No Apple Distribution identity is installed. Evidence: `artifacts/device-verification/2026-07-18-ios-ch13/`. |
...
| 2026-07-18 manual handoff recheck | IMPLEMENTATION_TEST_GO / ANDROID_COMPOSER_VERIFIED / IOS_SHARE_SHEET_VERIFIED | Android unit/lint/Debug/Release builds and iOS tests/current-source Apple Development device build pass. Pixel 9a reaches the normal ChatGPT composer with the ChatGPT-specific ZIP attached, empty question field, and no send. iOS current source reaches ZIP creation and the iOS share sheet via Appium/WDA; ChatGPT is not installed on that iPhone, so selection/composer remains unverified there. Public Privacy deploy and live verifier pass. |
...
| iPhone ChatGPT target/composer proof | Install the ChatGPT iOS app on the connected iPhone, then select it from the already verified share sheet and confirm the ZIP attachment, empty question field, and intentionally unsent state. This cannot be completed by the app alone and requires the owner-controlled App Store/account state. |
