...
| Supabase migration/replay | REMOTE_APPLIED_WITH_VALIDATION_WARNING | Local disposable replay reached `20260713223000_reconcile_promo_invite_code_events.sql` after compatibility repairs, but local container startup failed on the Colima Docker socket mount. The linked project accepted `20260712090000_purchase_security_binding.sql` and `20260713223000_reconcile_promo_invite_code_events.sql`; `supabase db query --linked` confirmed both versions. Linked lint/pgtap could not complete because the CLI temporary database role failed SASL authentication and `SUPABASE_DB_PASSWORD` is not configured. The pgtap compatibility assertions are recorded in `supabase/tests/promo_invite_code_event_compatibility_validation.sql`. |
