...
- このタグ詳細からの共有は、ホームから開く `AI` とは別機能。AI向けZIPには AI-safe Export Contract を適用する。
...
- 正式な入口はホーム右下の `タグ` / `アーカイブ` より上に置く独立した `AI`。エクスポート画面内のモード切替と下部バーの6個目は追加しない。`ChatGPT` / `Gemini` / `Claude` / `DeepSeek` をこの順で選べる。
- ユーザーは自作タグを1件以上選び、複数選択時はいずれかのタグに該当するURLを対象とする。共有タグは選択肢に出さない。
- 専用ページは自作タグ選択、対象件数、除外件数、選択したproviderへの送信を中心にし、全URL一覧や長い説明、別のZIP作成ボタン、確認チェックを常設しない。既知のemail/phone/token-like/Supabase/JWT/local-pathパターンは生成時に伏せ字にする。
