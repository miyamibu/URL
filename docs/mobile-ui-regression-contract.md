...
- `ChatGPT` は下部5アクションへ追加せず、ホーム右下の `タグ` / `アーカイブ` より上に独立したチャット導線として置く。
...
- 自作タグ名の編集はダブルタップに依存させず、長押し・メニュー・アクセシビリティアクション等から実行できるようにする。
...
- 正式な入口はホーム右下の `タグ` / `アーカイブ` より上に置く独立した `ChatGPT`。エクスポート画面内のモード切替と下部バーの6個目は追加しない。
