...
- Android 実機DBが schema 19 のまま、復元コードが schema 17 へ戻り、起動時に migration missing で落ちた。
...
6. Room/SQLite schema を戻す、または過去コミットへ復元する場合は、実機DBの `user_version` と migration 経路を確認する。
...
- 過去状態へコードを戻す場合でも、実機DBの schema が進んでいたら、保存済みデータを残す migration を先に用意する。
- schema を下げる必要がある場合は、追加列を落とすだけで済むかを確認し、URL/タグ本体を削除しない。
