...
- `AI` は下部5アクションへ追加せず、ホーム右下の `タグ` / `アーカイブ` より上に独立した導線として置く。押下後は `ChatGPT` / `Gemini` / `Claude` / `DeepSeek` の順で選択する。
...
- iPhone の上段行は、自作タグと既定サービスフィルター（YouTube / X / Instagram / TikTok / Web）を長押しドラッグで並べ替えでき、並び順を保持する。
- iPhone の上段行のドラッグ操作は追加/コピーではなく移動として扱い、コピーを示す `+` バッジを表示しない。
...
- このタグ詳細からの共有は、ホームから開く `ChatGPT` とは別機能。ChatGPT用ZIPには AI-safe Export Contract を適用する。
...
- 正式な入口はホーム右下の `タグ` / `アーカイブ` より上に置く独立した `AI`。エクスポート画面内のモード切替と下部バーの6個目は追加しない。`ChatGPT` を選んだ場合は以下の既存契約をそのまま適用し、他providerも同じeligible判定・preview・redaction・ZIP生成・OS共有を再利用する。
- ユーザーは自作タグを1件以上選び、複数選択時はいずれかのタグに該当するURLを対象とする。共有タグは選択肢に出さない。
- 専用ページは自作タグ選択、対象件数、除外件数、`ChatGPTに送る` を中心にし、全URL一覧や長い説明、別のZIP作成ボタン、確認チェックを常設しない。既知のemail/phone/token-like/Supabase/JWT/local-pathパターンは生成時に伏せ字にする。
- ChatGPT用ZIPに含めるのは、端末で保存した ACTIVE のURLのうち、削除待ちでなく、共有参照を持たないものだけ。対象が0件ならZIPを生成しない。
- `ChatGPTに送る` の押下を、画面に表示した対象件数に対する明示確認として扱う。押下後も直前のsnapshotを再検証し、選択や対象が変わった場合は共有せず再確認する。
...
- りんばむに質問入力欄を作らない。質問文の自動入力やChatGPT内の最終送信、OpenAI API/OAuth/login、MCP/provider接続も行わない。ChatGPT個人リンク同期/read-only MCPとは別機能として扱う。
