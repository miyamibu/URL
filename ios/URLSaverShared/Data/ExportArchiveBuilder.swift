...
private struct ExportSourceEntry {
    ...
    func toDocument(
        ...
        appliesExternalDataPolicy: Bool = true
    ) -> ExportEntryDocument {
        ...
        let externalURLReasons = appliesExternalDataPolicy
            ? ExternalDataPolicy.inspect(url: entry.normalizedURL).reasons
            : []
        exclusionReasons.append(contentsOf: externalURLReasons.map { "external_data_\($0)" })
        let externalTextReasons = appliesExternalDataPolicy
        ...
        let redactionApplied = Array(
            bodySummary.redactions
                ...
                .union(externalURLReasons)
                .union(externalTextReasons)
        ).sorted()
        func safeURL(_ value: String?) -> String? {
            guard let value, !value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                return nil
            }
            return appliesExternalDataPolicy ? ExternalDataPolicy.safeURL(value) : value
        }
        func safeText(_ value: String?) -> String? {
            guard let value, !value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                return nil
            }
            return appliesExternalDataPolicy ? ExternalDataPolicy.sanitizeText(value) : value
        }
        let safeTags = tags.map { tag in
            ExportTagSummary(
                id: tag.id,
                name: safeText(tag.name) ?? "[redacted:tag]",
                scope: tag.scope
            )
        }
        ...
        return ExportEntryDocument(
            ...
            originalUrl: safeURL(entry.originalURL) ?? ExternalDataPolicy.excludedURLMarker,
            normalizedUrl: safeURL(entry.normalizedURL) ?? ExternalDataPolicy.excludedURLMarker,
            displayUrl: safeURL(entry.displayURL) ?? ExternalDataPolicy.excludedURLMarker,
            openUrl: safeURL(entry.openURL) ?? ExternalDataPolicy.excludedURLMarker,
            providerPermalink: safeURL(URLExportArchiveBuilder.providerPermalink(for: entry)) ?? ExternalDataPolicy.excludedURLMarker,
            providerCanonicalId: safeText(entry.canonicalID),
            ...
            userTitle: safeText(entry.userTitle),
            fetchedTitle: safeText(entry.fetchedTitle),
            fetchedAuthorName: safeText(entry.fetchedAuthorName),
            ...
            canonicalId: safeText(entry.canonicalID),
            normalizedHost: safeText(entry.normalizedHost) ?? "保存したリンク",
            rawSourceHost: safeText(entry.rawSourceHost) ?? "",
            ...
            effectiveTitle: safeText(entry.effectiveTitle) ?? "保存したリンク",
    ...
    func toChatGptDocument(
        ...
        toDocument(
            ...
            appliesBaseRedaction: false,
            appliesExternalDataPolicy: false
        ).sanitizedForChatGpt(
            collisionOrdinal: collisionOrdinal,
            canonicalCreatedAt: canonicalCreatedAt
        )
